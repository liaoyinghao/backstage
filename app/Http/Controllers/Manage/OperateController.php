...
class OperateController extends Controller
{
    ...
    public function main(){
        $data['lists']=Operate::List();
        if(!empty($data['lists'][0])){
            foreach ($data['lists'] as $key => $val) {
                if(!empty($val['shopauthority'])){
                    $st = explode(',',$val['shopauthority']);
                    $data['lists'][$key]['number'] = count($st);
                }else{
                    $data['lists'][$key]['number'] = 0;
                }
                
            }
        }
        return view('manage.operate.main' , $data);
    }
    ...
    public function store()
    {
        ...
        $login =Operate::where('id',$id)->first();
        $data['id']=$id;
        //权限勾选
        $storeauthority=Operate::where('id',$id)->first();
        ...
        return view('manage.operate.store',$data);
    }
    ...
    public function storeAuthority(Request $request)
    {
        ...
        return redirect()->route('manage_operate_main');
    }
