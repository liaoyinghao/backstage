        ...
        <div class="col-md-12">
            <div class="portlet light bordered">
                ...
                <div class="portlet-body">
                    ...
                    <form method="post" action="{{route('manage_exproject_addprojectpost')}}">
                    <input type="hidden" name="id" value="{{$start['id'] or ''}}">
                    <p class="form_p">
                        <span class="userk">名称：</span><input type="text" name="start[proname]" value="{{$start['proname'] or ''}}" placeholder="名称">
                    </p>
                    <p class="form_p">
                        <span class="userk">所需材料：</span>
                        <textarea name="start[material]" placeholder="所需材料" class="textarea" style = "">{{$start['material'] or ''}}</textarea>
                    </p>
                    <p class="form_p">
                        <span class="userk">底价金额：</span><input type="number" name="start[price]" step="0.01" value="{{$start['price'] or ''}}" placeholder="输入金额" required="required">
                    </p>
                    <p class="form_p">
                        <span class="userk">要求：</span>
                        <textarea name="start[demand]" placeholder="要求" class="textarea">{{$start['demand'] or ''}}</textarea>
                    </p>
                    <p class="form_p">
